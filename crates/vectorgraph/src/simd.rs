...
pub(crate) fn dot_f16(left: &[f32], right_le_bytes: &[u8]) -> f32 {
    debug_assert_eq!(left.len() * 2, right_le_bytes.len());
...
unsafe fn dot_f16_neon(left: &[f32], right_le_bytes: &[u8]) -> f32 {
    ...
    while index + 8 <= left.len() {
        ...
        // Stable Rust does not yet expose `vcvt_f32_f16`; these are the exact
        // AArch64 FCVTL operations used by that intrinsic.
        unsafe {
            asm!(
