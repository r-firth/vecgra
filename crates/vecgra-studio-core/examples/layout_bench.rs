...
fn separation_metrics(positions: &[Vec2]) -> SeparationMetrics {
    ...
    let bounds = vecgra_studio_core::Rect::from_points(positions);
...
fn main() {
    ...
    let path = arguments
        .next()
        .map(PathBuf::from)
        .expect("usage: cargo run -p vecgra-studio-core --example layout_bench --release -- <graph.vg> [force|structure]");
