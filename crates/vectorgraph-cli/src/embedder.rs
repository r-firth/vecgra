...
impl OpenRouterEmbedder {
    fn qwen(dimension: usize, request_batch_size: usize) -> Result<Self, Box<dyn Error>> {
        ...
        let concurrency = env::var("VG_EMBEDDING_CONCURRENCY")
            .ok()
            .map(|value| value.parse::<usize>())
            .transpose()
            .map_err(|error| format!("invalid VG_EMBEDDING_CONCURRENCY: {error}"))?
            .unwrap_or(4)
            .clamp(1, 32);
        Ok(Self {
...
pub(crate) fn feature_vector(text: &str, dimension: usize) -> Vec<f32> {
    vectorgraph_embedding::feature_vector(text, dimension)
}
