    ...
    match command.as_str() {
        ...
        "import-rust" => {
            ...
            let embedder_name = arguments
                .next()
                .or_else(|| env::var("VG_EMBEDDER").ok())
        ...
        "bench-search" => {
            ...
            if let Some(candidates) = candidate_vectors {
                ...
                println!(
                    "recall_at_10\t{:.3}",
                    recall(&exact_hits.unwrap(), &first_hits)
                );
            }
...
fn display_value(read: &vectorgraph::ReadGuard<'_>, value: &Value) -> String {
    match value {
...
fn recall(exact: &[vectorgraph::VectorHit], approximate: &[vectorgraph::VectorHit]) -> f64 {
    if exact.is_empty() {
        return 1.0;
    }
...
fn print_help() {
    println!(
        "VectorGraph CLI\n\n\
         Usage:\n\
           vg import-jsonl <nodes.jsonl> <edges.jsonl> <database> <dimension> [f16|f32]\n\
           vg import-fbin <train.fbin> <database>\n\
           vg import-node-fbin <train.fbin> <metadata.jsonl> <database> [f16|f32]\n\
           vg bench-fbin <database> <test.fbin> <neighbors.ibin> [queries] [candidate-vectors] [k] [compressed|hot-f32]\n\
           vg bench-filtered-fbin <database> <test.fbin> [queries] [stride] [candidate-elements] [k]\n\
           vg bench-range-fbin <database> <test.fbin> <neighbors.ibin> <property> <inclusive-lower-bound> [queries] [candidate-elements] [k]\n\
           vg import-graphalytics <vertices> <edges> <database>\n\
           vg bench-bfs <database> <source> [reference-output|-] [iterations]\n\
           vg bench-wcc <database> [reference-output|-] [iterations]\n\
           vg bench-pagerank <database> [reference-output|-] [benchmark-iterations]\n\
           vg import-rust <repository> <database> [dimension] [hash|qwen] [batch-size]\n\
           vg import-github <owner/repository> <database> [issues] [pulls] [discussions] [releases] [dimension] [hash|qwen] [batch-size]\n\
           vg stats <database>\n\
           vg check <database>\n\
           vg plan-search <database> [nodes|edges|both] [label]\n\
           vg bench-property <database> <nodes|edges|both> <key> <json-scalar> [iterations]\n\
           ...
           vg semantic-text <database> <query> [limit] [max-hops] [hash|qwen]\n\
           vg bench-search <database> <query> [iterations] [hash|qwen] [nodes|edges|both] [label|-] [candidate-vectors]\n\n\
           vg bench-ann <database> [queries] [candidate-vectors] [nodes|edges|both]\n\n\
           vg query <database> 'MATCH (a:A)-[e:E]->(b:B) RETURN a,e,b LIMIT 10'\n\n\
           vg query-text <database> '<MATCH query>' <semantic-text> [hash|qwen]\n\n\
           vg bench-pattern <database> <query> [iterations]\n\n\
           vg bench-neighbors <database> <node-id> [iterations] [out|in|both]\n\n\
           vg bench-expand <database> <seed-node-id> <hops> [iterations] [out|in|both]\n\n\
           vg compact <source-database> <destination-database> [f16|f32]\n\n\
           vg export-ladybug <database> <output-directory>\n\n\
         OPENROUTER_API_KEY is required for qwen. VG_EMBEDDER sets the default embedder."
    );
}
