...
use std::path::Path;
use std::sync::Arc;
use vectorgraph::{BulkLoader, DatabaseOptions, GraphStats, Similarity, Value, VectorEncoding};
...
mod tests {
    ...
    use vectorgraph::{Database, Direction, EdgeFilter};
    ...
    fn path(suffix: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "vectorgraph-jsonl-{}-{nonce}-{suffix}",
            std::process::id()
        ))
    }
    ...
    fn node_fbin_import_streams_vectors_and_typed_metadata_in_lockstep() {
        ...
        assert_eq!(
            read.elements_with_label(movie, vectorgraph::VectorTarget::Nodes)
                .len(),
            2
        );
        let first = read.node(0).unwrap();
