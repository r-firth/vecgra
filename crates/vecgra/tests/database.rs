...
fn temp_database(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("vecgra-{name}-{}-{nonce}.vg", std::process::id()))
}
...
fn append_raw_frame(path: &PathBuf, payload: &[u8], operation_count: u32) {
    let mut fixed = [0u8; 32];
    fixed[..8].copy_from_slice(b"VGRTXN01");
    fixed[8..16].copy_from_slice(&(payload.len() as u64).to_le_bytes());
    fixed[16..24].copy_from_slice(&1u64.to_le_bytes());
    fixed[24..28].copy_from_slice(&operation_count.to_le_bytes());

    let mut checksum_input = Vec::from(&fixed[8..]);
    checksum_input.extend_from_slice(payload);
    let mut file = OpenOptions::new().append(true).open(path).unwrap();
    file.write_all(&fixed).unwrap();
    file.write_all(payload).unwrap();
    file.write_all(&test_crc32c(&checksum_input).to_le_bytes())
        .unwrap();
    file.write_all(&0x5647_454eu32.to_le_bytes()).unwrap();
    file.sync_all().unwrap();
}

#[test]
fn arbitrary_short_files_are_rejected_without_panicking() {
    let path = temp_database("arbitrary-input");
...
#[test]
fn torn_frame_length_is_bounded_by_bytes_remaining_before_allocation() {
    let path = temp_database("claimed-frame-length");
    drop(Database::create(&path, options()).unwrap());

    let mut fixed = [0u8; 32];
    fixed[..8].copy_from_slice(b"VGRTXN01");
    fixed[8..16].copy_from_slice(&(2u64 * 1024 * 1024 * 1024).to_le_bytes());
    fixed[16..24].copy_from_slice(&1u64.to_le_bytes());
    OpenOptions::new()
        .append(true)
        .open(&path)
    ...
    assert_eq!(database.read().stats().nodes, 0);
...
#[test]
fn corrupt_operation_and_property_counts_do_not_drive_allocations() {
    let operation_path = temp_database("operation-count");
    drop(Database::create(&operation_path, options()).unwrap());
    append_raw_frame(&operation_path, &[], u32::MAX);
    assert!(Database::open_read_only(&operation_path).is_err());
    std::fs::remove_file(operation_path).unwrap();

    let property_path = temp_database("property-count");
    drop(Database::create(&property_path, options()).unwrap());
    let mut payload = Vec::new();
    payload.push(2); // PutNode
    payload.extend_from_slice(&0u64.to_le_bytes()); // id
    payload.extend_from_slice(&0u64.to_le_bytes()); // generation
    payload.extend_from_slice(&0u32.to_le_bytes()); // label
    payload.extend_from_slice(&u32::MAX.to_le_bytes()); // property count
    append_raw_frame(&property_path, &payload, 1);
    assert!(Database::open_read_only(&property_path).is_err());
    std::fs::remove_file(property_path).unwrap();
}

#[test]
fn overflowing_checkpoint_offsets_are_rejected_in_the_header() {
    let path = temp_database("overflowing-offsets");
    drop(Database::create(&path, options()).unwrap());
    let mut header = std::fs::read(&path).unwrap();
    header[24..32].copy_from_slice(&20u64.to_le_bytes());
    header[32..40].copy_from_slice(&84u64.to_le_bytes());
    header[40..48].copy_from_slice(&u64::MAX.to_le_bytes());
    let checksum = test_crc32c(&header[..60]);
    header[60..64].copy_from_slice(&checksum.to_le_bytes());
    std::fs::write(&path, header).unwrap();

    let error = Database::open_read_only(&path).unwrap_err().to_string();
    assert!(error.contains("invalid checkpoint section offsets"));
    std::fs::remove_file(path).unwrap();
}
...
fn graph_candidate_sets_fuse_traversal_labels_and_vector_search() {
    ...
    let read = database.read();
    let roots =
        read.elements_with_label(read.label_id("Root").unwrap(), vecgra::VectorTarget::Nodes);
    let documents = read.elements_with_label(
        read.label_id("Document").unwrap(),
        vecgra::VectorTarget::Nodes,
    );
    let expanded = read
...
fn shortest_path_is_bounded_filtered_deterministic_and_checkpoint_stable() {
    ...
    assert!(matches!(
        ...
        Err(vecgra::Error::NotFound("node", u64::MAX))
    ));
